...
class LAVA(GraphsOperator):
    def __init__(self):
        ...
        output_file = os.path.join("output", "positions")

        os.makedirs(output_file, exist_ok=True)
        os.makedirs("input", exist_ok=True)

        text_recogniser_path = os.path.join("assets", "tracking", "crnn.onnx")
    ...
    def headers(self):
        headers_path = os.path.join(tracking_folder, "headers")

        headers_list = os.listdir(headers_path)

        header_templates = {}
        for header in headers_list:
            header_directory = os.path.join(headers_path, header)
        ...
        while ret is True and header_found is False:
            ...
            for header in header_templates.keys():
                header_max_probability = np.max(
                    cv2.matchTemplate(
                        cropped, header_templates[header], cv2.TM_CCOEFF_NORMED
                    )
                )
            ...
            for role in self.roles:
                ...
                for j, template in enumerate(blue_champ_templates):
                    champ_classify_percent = np.max(
                        cv2.matchTemplate(blue_crop, template, cv2.TM_CCOEFF_NORMED)
                    )
        ...
        while identified != 10:
            ...
            for role in self.roles:
                ...
                for j, template in enumerate(red_champ_templates):
                    champ_classify_percent = np.max(
                        cv2.matchTemplate(red_crop, template, cv2.TM_CCOEFF_NORMED)
                    )
        ...
        for side in champs:
            for role in champs[side]:
                ...
                champ_image = os.path.join(
                    tracking_folder, "champ_tracking", f"{champ}.jpg"
                )
                ...
                for i in range(2):
                    ...
                    for summoner in summoners.keys():
                        max_probability = np.max(
                            cv2.matchTemplate(
                                cropped, summoners[summoner], cv2.TM_CCOEFF_NORMED
                            )
                        )
    ...
    def map_borders(self, cap, league):
        ...
        if league == "automatic":
            for i in range(4):
                inhib_path = os.path.join(tracking_folder, "minimap", f"inhib{i}.jpg")
            ...
            while ret is True and inhibs_found < 4:
                ...
                if inhibs_found == 4:
                    swapped_inhibs = [
                        coord[::-1]
                        for coord in inhib_locations[2:] + inhib_locations[:2]
                    ]
            ...
            return (
                slice(height // 2 + int(border2[1]), height // 2 + int(border1[1])),
                slice(
                    4 * width // 5 + int(border1[0]), 4 * width // 5 + int(border2[0])
                ),
            )
        else:
            borders = leagues[league]

            return (slice(borders[0], borders[1]), slice(borders[2], borders[3]))
        ...
        if self.lightweight is True:
            ...
            for image_i in self.digit_templates:
                res = cv2.matchTemplate(
                    cropped_timer, self.digit_templates[image_i], cv2.TM_CCOEFF_NORMED
                )
            ...
            blob = cv2.dnn.blobFromImage(
                cropped, size=(100, 32), mean=127.5, scalefactor=1 / 127.5
            )
                ...
                for side in champs.keys():
                    for role in champs[side].keys():
                        ...
                        try:
                            point = next(zip(*location[::-1]))
                            cv2.rectangle(
                                cropped, point, (point[0] + 14, point[1] + 14), 255, 2
                            )
                        except:
                            point = (np.nan, np.nan)
                            pass
    ...
    def interpolate(df_input):
            ...
            if np.all(np.all(np.isnan(colt))):
                ...
                if np.all(np.isnan(col[len(col) - 1])):
                    try:
                        ...
                        if found:
                            col_temp = pd.concat(
                                [(col_temp[: j + 1]), (df[champ_found][j + 1 :])]
                            )
                    except:
                        pass
                ...
                while k < len(col_temp2) - 1:
                    ...
                    if np.all(np.isnan(col_temp[k])):
                        ...
                        if count > 5:  # Missing for more than 5 seconds
                            ...
                            if (
                                norm(np.array(point) - np.array([circle_x, circle_y]))
                                < RADIUS
                            ):
        ...
        df_merged = pd.merge(df_input, df_melted, on=["champ", "iter"]).drop(
            ["coords", "iter"], axis=1
        )
