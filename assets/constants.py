...
tracking_folder = os.path.join("assets", "tracking")

blue_classify_path = os.path.join(tracking_folder, "champ_classifying", "blue")

red_classify_path = os.path.join(tracking_folder, "champ_classifying", "red")

roles = ["top", "jgl", "mid", "adc", "sup"]

summoner_path = os.path.join(tracking_folder, "summoners")

summoners = {}

for summoner in os.listdir(summoner_path):
    summoner_image_path = os.path.join(summoner_path, summoner)
...
timer_borders = {
    "default": {
        "minute": (slice(52, 63), slice(628, 641)),
        "second": (slice(52, 63), slice(644, 658)),
    }
}
...
if os.path.exists(blue_classify_path):
    ...
    for portrait_i, portrait in enumerate(blue_portraits):
        portrait_image = os.path.join(blue_classify_path, portrait)

        blue_champ_templates[portrait_i] = cv2.imread(portrait_image, 0)

    for portrait_i, portrait in enumerate(red_portraits):
        portrait_image = os.path.join(red_classify_path, portrait)
