...
class GraphsOperator:
    def draw_graphs(self, df, video):
        ...
        image_path = os.path.join(output_path, "level_one.svg")
        ...
        for role in ["jungle", "support", "mid"]:
            for side in ["blue", "red"]:
                image_path = os.path.join(output_path, f"{side}_{role}.svg")
        ...
        graphs["blue_prox.svg"], graphs["red_prox.svg"] = self.proximity_graphs(df)

        for side in graphs:
            image_path = os.path.join(output_path, side)
    ...
    def proximity_graphs(self, df):
        ...
        for side in ["red", "blue"]:
            fig = plotly.subplots.make_subplots(
                rows=1,
                cols=3,
                subplot_titles=["0-8 Minutes", "8-14 Minutes", "14-20 Minutes"],
            )
            for i, subplot in enumerate(subplots.keys()):
                df_means = (
                    df[
                        (df.side == side)
                        & (df.second > subplots[subplot][0])
                        & (df.second < subplots[subplot][1])
                    ]
                    .groupby(["champ", "role"])["x", "y"]
                    .mean()
                )
                ...
                for champ in champs:
                    image_path = os.path.join(
                        "assets", "graphing", "portraits", f"{champ.title()}Square.png"
                    )
                ...
                for combo in combos:
                    ...
                    dft = (
                        df_means_reset[df_means_reset.role.isin(combo_roles)][
                            ["x", "y"]
                        ]
                    ).values.tolist()
            ...
            fig.update_layout(
                height=500, width=1200, title_text="Proximities and Average Positions"
            )
    ...
    def classify_sup(x, regions):
        ...
        if norm(point - np.array([1, 0])) < 0.4:  # Red base
            ...
            regions[3] += 1
        # Botlane
        elif norm(point - np.array([1, 1])) < 0.4 or point[0] > 0.9 or point[1] > 0.9:
            regions[5] += 1
        elif point[0] < 0.9 - point[1]:  # Above midlane upper border
            regions[0] += 1
        # Above midlane lower border (in midlane)
    ...
    def classify_mid(x, regions):
        ...
        # Botlane
            ...
            regions[5] += 1
        # Toplane
        elif norm(point - np.array([0, 1])) < 0.4 or point[0] < 0.1 or point[1] < 0.1:
            regions[4] += 1
    ...
    def jungleplots(self, df, colour, image_path):
        ...
        for subplot in subplots.keys():
            df_timesplit = df_side[
                (df_side.second >= subplots[subplot][0])
